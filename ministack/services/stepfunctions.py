...
def _pascal_to_camel(key):
    """Convert PascalCase to camelCase: 'ResourceArn' -> 'resourceArn'."""
    if not key:
        return key
...
def _convert_keys_to_camel(data):
    ...
    if isinstance(data, dict):
        return {_pascal_to_camel(k): _convert_keys_to_camel(v) for k, v in data.items()}
    if isinstance(data, list):
