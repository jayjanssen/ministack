...
- **Step Functions — execution history pagination** — `GetExecutionHistory` accepts `nextToken` and returns successive pages in forward or reverse order, with a token only while more events remain. `includeExecutionData=false` omits input and output payloads from returned history events.
...
- **CloudFormation — AppSync GraphQL APIs and API keys update in place** — every property of `AWS::AppSync::GraphQLApi` is No interruption, but a change re-ran the create, which minted a new api id with empty key, data source, resolver and type stores, so a renamed API came back empty. The API now keeps its id and applies the change in place. It stores `AdditionalAuthenticationProviders`, `XrayEnabled`, `QueryDepthLimit`, `ResolverCountLimit`, `IntrospectionConfig`, `EnvironmentVariables`, `OwnerContact`, `LogConfig`, `UserPoolConfig`, `OpenIDConnectConfig`, `LambdaAuthorizerConfig`, `EnhancedMetricsConfig` and `MergedApiExecutionRoleArn` in AppSync's camelCase, and a removed member goes back to AppSync's default. A `Visibility` change fails with AWS's message. The API carries the stack-level tags, its id has `CreateGraphqlApi`'s 26 characters (eight made an ARN botocore refuses) and it answers the `REALTIME` endpoint. `AWS::AppSync::ApiKey` keeps its key on a `Description` or `Expires` change, and its record carries `description` and `createdAt`, as `CreateApiKey` writes them. Contributed by @iot-rocket.
- **CloudFormation — Auto Scaling groups, scaling policies and scheduled actions update in place** — a change re-ran the create, which mints a new ARN from a uuid, and the group also lost its instances and its creation time. The three types now update in place. An `AutoScalingGroupName` or `InstanceId` change replaces the group, and a group tag change keeps tags added through `CreateOrUpdateTags`. As on AWS, `PolicyName` names a scaling policy at create and is ignored after that, and CloudFormation names a scheduled action itself, ignores `ScheduledActionName` and answers the generated name through `Ref` (it answered the ARN). `DescribeAutoScalingGroups` answers the launch template's name beside its id. Contributed by @iot-rocket.
- **CloudFormation — a VPC updates in place and is deleted with its default children** — `EnableDnsSupport`, `EnableDnsHostnames` and `Tags` are No interruption, but a change re-ran the create, which mints a new `vpc-` id. The delete removed only the VPC, so each change left its default security group, main route table and network ACL behind. The VPC now updates in place, and a removed DNS property goes back to its default. The delete goes through `DeleteVpc`, which removes the default children and fails with `DependencyViolation` while a security group created outside the stack is still in the VPC; AWS keeps that VPC too and retries the delete for a long time. The create applies the two DNS properties and the template's tags, which it ignored. `Ipv4IpamPoolId`, `Ipv4NetmaskLength` and `VpcEncryptionControl` replace the VPC, like `CidrBlock`. Contributed by @iot-rocket.
- **CloudFormation — a subnet updates in place** — `MapPublicIpOnLaunch`, `PrivateDnsNameOptionsOnLaunch`, `EnableDns64` and `Tags` are No interruption, but a change re-ran the create, which mints a new `subnet-` id and left every instance, network interface and NAT gateway on an id that no longer resolves. They are applied in place now, and a property the template removes keeps its value, as on AWS. `EnableDns64` without an IPv6 block fails with AWS's message. `AvailabilityZoneId` is honoured, and it, the IPAM members, `Ipv6Native` and `OutpostArn` replace the subnet. A `MapPublicIpOnLaunch` of `"false"` reads false. Contributed by @iot-rocket.
- **CloudFormation — a security group updates its rules in place** — `SecurityGroupIngress` and `SecurityGroupEgress` are updated in place on AWS and `Tags` is No interruption, but a change re-ran the create, which mints a new `sg-` id and lost every rule authorized outside the template. The rules are reconciled now: dropped ones are removed, new ones added, a changed `Description` is applied, and rules added outside the template stay. The create applies `SecurityGroupEgress`, which it ignored, and adds the allow-all egress rule only when the template declares none. An update that declares egress removes that rule, and one that drops the declaration leaves no egress rule, as on AWS. A rule's `Description`, `CidrIpv6`, prefix-list source or destination and a source group's name and owner are kept. Contributed by @iot-rocket.
- **CloudFormation — a launch template change adds a version** — `LaunchTemplateData`, `TagSpecifications` and `VersionDescription` are No interruption, but a change re-ran the create, which minted a new `lt-` id and reset `LatestVersionNumber` and `DefaultVersionNumber` to 1. Each change now adds a version to the same template and the default version stays where it is, as measured on AWS. A rollback adds a version with the old data. The template's own tags come from its `TagSpecifications` entry for `launch-template` and are applied at create only (the create read a `Tags` property the type does not have). Contributed by @iot-rocket.
- **CloudFormation — an internet gateway keeps its id and its attachment** — `Tags` is the only property of `AWS::EC2::InternetGateway` and it is No interruption, but the create ignored it, so a tag change minted a new `igw-` id, reset `Attachments` and applied no tags. The gateway now keeps its id and attachment and its tags are applied. Contributed by @iot-rocket.
- **CloudFormation — a route table keeps its routes and associations** — `Tags` is No interruption on `AWS::EC2::RouteTable`, but a tag change re-ran the create, which minted a new `rtb-` id with only the local route and no associations, so everything added through `CreateRoute` or `AssociateRouteTable` was lost. The table now keeps its id, routes and associations, and its tags are applied. Contributed by @iot-rocket.
- **CloudFormation — a route target change updates the route in place** — every target property of `AWS::EC2::Route` is No interruption, but the physical id `"{table}|{destination}"` did not move, so the create appended a second route for the same destination. The update and the create now replace the route for that destination, and the ten targets the create dropped are read, `OdbNetworkArn` included. The destination is read from `DestinationCidrBlock`, `DestinationIpv6CidrBlock` or `DestinationPrefixListId`: an IPv6 or prefix-list route overwrote the IPv4 default route, where AWS keeps them side by side. Contributed by @iot-rocket.
...
- **EC2 — `DescribeRouteTables` answers every route destination and target** — an IPv6 or prefix-list route came back with an empty `destinationCidrBlock`, and only five target types were rendered. All destinations and targets are answered now, a VPC endpoint target as `gatewayId`. Contributed by @iot-rocket.
