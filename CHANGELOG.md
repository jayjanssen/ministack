...
- **CloudFormation — the `AWS::Include` transform** — an embedded `Fn::Transform` naming `AWS::Include` was carried into the stack as a literal key; it is now replaced by the contents of the S3 object its `Location` points to (JSON or plain YAML, a key-value object) before the template is validated: next to a sibling resource inside `Resources`, as a whole `Properties` map, inside one next to other properties, in `Mappings`, or in `Outputs`. A location that is not an `s3://` URI, a snippet with a YAML shorthand tag, a missing object, a snippet that is not an object and a nested include are refused before a stack exists, with the sentence a real account answers where one is known. `UsePreviousTemplate` and a change set without a body reuse the stored processed template, so a snippet edited in S3 since the deploy is not picked up, as on AWS. Contributed by @iot-rocket.
- **Amazon Location — trackers and device positions, with `AWS::Location::Tracker`** — the service did not exist. `CreateTracker`, `DescribeTracker`, `UpdateTracker`, `ListTrackers` and `DeleteTracker` manage trackers; `BatchUpdateDevicePosition`, `GetDevicePosition`, `BatchGetDevicePosition` and `GetDevicePositionHistory` serve device positions, with the modeled wire shapes (`geo` signing scope, ISO 8601 timestamps, `Position` as the `[lon, lat]` pair, per-entry `Errors` for out-of-range coordinates, `NextToken` paging, the last 24 hours as the history default). `PositionFiltering` applies: `TimeBased` (the default) stores one sample per 30 seconds per device, `DistanceBased` ignores a move under 30 m and `AccuracyBased` a move under the two updates' measured accuracy, and a filtered update is not an error. The documented request constraints are checked before the tracker is looked up, so an invalid request naming a tracker that does not exist answers the `ValidationException`, not the 404, and `TrackerName` is held to its 1..100 length and to the ASCII the service-side Java `[-._\w]+` means, in the URL as well as in a body; `Accuracy` is the modeled `PositionalAccuracy`, whose required `Horizontal` runs 0..10000000 metres and can no longer arrive negative under the `AccuracyBased` filter it feeds; every `DeviceId` is the modeled `Id`, in an update, in a `DeviceIds` list and in the URL alike; a `GetDevicePositionHistory` window bound that is not a timestamp is refused instead of falling back to the 24-hour default. Positions are kept in memory, the newest 100 per device, rather than for the service's 30 days; `KmsKeyId`, `EventBridgeEnabled` and `Tags` are stored without effect; no consumers, geofences, maps, places or routes. The CloudFormation resource `Ref`s the name, exposes `Arn`/`TrackerArn`/`CreateTime`/`UpdateTime`, updates in place and receives the stack tags. Contributed by @iot-rocket.
- **AWS Signer — synchronous `StartSigningJob` with the S3 side effect** — the `signer` API did not exist. `StartSigningJob`, `DescribeSigningJob`, `ListSigningJobs`, `PutSigningProfile` and `GetSigningProfile` are served natively (REST-JSON). Signing is synchronous and synthetic: Start validates the source object against the local S3 store, writes a JSON marker (source reference + SHA-256 of the source bytes) to `destination.prefix + jobId` (`.zip` appended for a `.zip` source on the Lambda platform, as measured) and returns the job already `Succeeded`; a missing source, destination bucket or profile is `ResourceNotFoundException` at Start with no job recorded. `clientRequestToken` replays the first response. Jobs carry the profile's signing material and a `signatureExpiresAt`; list filters and the documented input constraints are enforced, and `ListSigningJobs` pages with a `nextToken` that is bound to the filters it was minted with. The unsigned path rules leave path-style S3 traffic for a bucket named `signing-jobs` alone unless the request looks like signer's own surface, and a job records the IAM principal that started it rather than the account root. `signatureValidityPeriod` follows the model, where both members are optional, and the documented constraints on `overrides`, `signingMaterial`, `tags`, `jobInvoker` and `profileOwner` are enforced instead of stored as sent. State persists with `PERSIST_STATE=1`. Contributed by @iot-rocket.
- **IoT Wireless — `GetPositionEstimate`** — the service was absent. `POST /position-estimate` now answers the way the API is shaped: the output structure declares a `payload` blob, so the HTTP body is the raw GeoJSON `Point` and boto3 hands the caller a `StreamingBody`, not a JSON envelope. The estimate is synthetic and deterministic (the canonical `Ip.IpAddress` hashes onto lon [-180, 180) / lat [-60, 60)); `properties` carry the two accuracy fields plus `timestamp`, which is always there: AWS documents the request's `Timestamp` as the time the position is resolved and the receive time as its substitute when the caller sends none, so a request that carries a `Timestamp` answers byte-identical bytes. `WiFiAccessPoints`, `CellTowers` and `Gnss` are accepted and ignored, while `AdvancedConfiguration.WiFiCellular.ConfidencePercent` drives the reported `horizontalConfidenceLevel` over 100, within the 50 to 99 the model allows. Refusals carry the live service's wording: no measurement, an unparseable address, a `Timestamp` that is not a Unix timestamp or a `ConfidencePercent` out of range is `ValidationException`; an address that is not globally routable, or is multicast, is `ResourceNotFoundException` `Cannot find position for the IP address <ip>`, as is a well-formed request whose only measurements are WLAN/cell/GNSS, which AWS documents that class for. Stateless. Contributed by @iot-rocket.

### Fixed
- **EC2 — `AvailabilityZoneId` is populated on subnets** — `DescribeSubnets` and `CreateSubnet` always returned `null` for `AvailabilityZoneId`, even though `DescribeAvailabilityZones` reports the correct `ZoneId` mapping for the same account/region. Real AWS always populates this field, and consumers that fall back to computing it themselves when it's missing (e.g. the AWS Load Balancer Controller's subnet resolution) can crash on the fallback path. `CreateSubnet` now derives the id from the given (or default) `AvailabilityZone`; an `AvailabilityZoneId` supplied alongside it is preserved as given. The default-VPC and CloudFormation (`AWS::EC2::Subnet`) subnet-creation paths derive it the same way, and `DescribeSubnets` now always returns a non-null value. Subnets restored from state saved before this fix are backfilled on load. Contributed by @Alexis-DevOps.
- **EC2 — `DescribeAvailabilityZones` reports `ZoneType`** — the field was omitted entirely, so a consumer that branches on it (the AWS Load Balancer Controller's subnet locale resolution) read it as empty and refused to classify the subnet ("unknown zone type"), blocking Service/Ingress reconciliation. Every zone ministack fabricates is a standard Availability Zone, so `DescribeAvailabilityZones` now always reports `"availability-zone"`. Contributed by @Alexis-DevOps.
- **CloudFormation — the template and stack quotas are enforced** — a template of any size, with any number of resources, parameters, outputs or mappings, and a stack name of any shape were accepted, so a template that a real account refuses deployed. `CreateStack`, `UpdateStack`, `CreateChangeSet`, `ValidateTemplate` and `GetTemplateSummary` now refuse a `TemplateBody` over 51,200 bytes and a template behind `TemplateURL` over 1,000,000 bytes; the engine refuses more than 500 resources, 200 parameters, 200 outputs, 200 mappings or 200 attributes in a mapping, a name over 255 characters, a description over 1,024 bytes and a parameter default or value over 4,096 bytes; `CreateStack` and a `CREATE` change set refuse a stack name that is not alphanumeric-and-hyphens starting with a letter or longer than 128 characters, joined with an oversized body into the one message the API's parameter validation gives. All of it before a stack record exists, with the messages of the API's parameter validation and the template format errors. `ValidateTemplate` also takes `TemplateURL` now. Contributed by @iot-rocket.
### Fixed
